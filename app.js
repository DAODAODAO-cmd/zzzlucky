(() => {
  ...
  const elements = {
    ...
    addPrizeButton: $("addPrizeButton"), groupPrizeName: $("groupPrizeName"), groupPrizeQuantity: $("groupPrizeQuantity"), groupTotalCards: $("groupTotalCards"),
    ...
    roomCodeDisplay: $("roomCodeDisplay"), prizeLegend: $("prizeLegend"), remainingLabel: $("remainingLabel"), prizeStatLabel: $("prizeStatLabel"),
  ...
  function addEditorRow(container, kind, name = "", quantity = 1) {
    ...
    count.addEventListener("input", updateCreateUI); row.append(text, count, remove); container.append(row); updateCreateUI();
  }
  ...
  function readRows(container, nameClass, qtyClass) { return [...container.children].map(row => ({ name: cleanName(row.querySelector(`.${nameClass}`).value), quantity: Number(row.querySelector(`.${qtyClass}`).value) })).filter(item => item.name); }
  ...
  addEditorRow(elements.prizeEditor, "prize", "A 款", 2); addEditorRow(elements.prizeEditor, "prize", "B 款", 2); addEditorRow(elements.personEditor, "person", "", 1);
  ...
  function renderRoom() {
    ...
    elements.adminPanel.classList.toggle("hidden", !state.ownerToken); elements.drawWinnersButton.classList.toggle("hidden", !isRaffle || room.phase === "drawn"); elements.toggleRoomButton.classList.toggle("hidden", isRaffle && room.phase === "drawn"); elements.toggleRoomButton.textContent = room.is_open ? (isRaffle ? "暂停报名" : "暂停抽选") : (isRaffle ? "继续报名" : "继续抽选");
  ...
  function animateCard(cardNumber, result) { return new Promise(resolve => { const card = elements.cardGrid.querySelector(`[data-card="${cardNumber}"]`); if (!card) return resolve(); const back = card.querySelector(".card-back"); back.classList.toggle("winner", Boolean(result.prize_name)); back.querySelector(".result-icon").textContent = result.prize_name ? "✦" : "○"; back.querySelector("strong").textContent = state.name; back.querySelector("span:last-child").textContent = result.prize_name || "本次未抽中"; requestAnimationFrame(() => { card.classList.add("flipped", "just-flipped"); if (navigator.vibrate) navigator.vibrate(result.prize_name ? [35,35,70] : 25); }); setTimeout(resolve, 850); }); }
  ...
  async function drawRaffleWinners() { if (!confirm("确定结束报名并立即开奖吗？开奖后名单不能继续增加。")) return; try { await rpc("draw_lottery_winners", { p_room_code: state.code, p_owner_token: state.ownerToken }); await loadRoom(true); launchCelebration(); showToast("开奖完成"); } catch (error) { showToast(error.message); } }

  function showReveal(result) { const won = Boolean(result.prize_name); elements.revealKicker.textContent = `第 ${result.card_number} 个盒子`; elements.revealTitle.textContent = won ? `抽到「${result.prize_name}」啦！` : "这个盒子没有礼物"; elements.revealText.textContent = won ? "结果已经记录，可以继续看看大家开出了什么。" : "本次没有抽中，结果也会保留在开盒记录里。"; elements.revealIcon.textContent = won ? "✦" : "○"; elements.revealModal.querySelector(".reveal-box").classList.toggle("win", won); elements.revealModal.classList.remove("hidden"); if (won) launchCelebration(); elements.closeRevealButton.focus(); }
  function launchCelebration() { if (matchMedia("(prefers-reduced-motion: reduce)").matches) return; const colors = ["#ffcf68","#8b78dc","#ff8c91","#56b995","#ffffff"], fragment = document.createDocumentFragment(); for (let i=0;i<42;i+=1) { const p=document.createElement("i"); p.className="confetti"; p.style.setProperty("--x",`${Math.random()*100}%`); p.style.setProperty("--w",`${5+Math.random()*7}px`); p.style.setProperty("--drift",`${-90+Math.random()*180}px`); p.style.setProperty("--spin",`${360+Math.random()*720}deg`); p.style.setProperty("--duration",`${1.7+Math.random()*1.2}s`); p.style.setProperty("--delay",`${Math.random()*.35}s`); p.style.setProperty("--color",colors[i%colors.length]); fragment.append(p); } elements.celebrationLayer.replaceChildren(fragment); setTimeout(()=>elements.celebrationLayer.replaceChildren(),3400); }
