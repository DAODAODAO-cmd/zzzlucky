(() => {
  ...
  function renderPrizeLegend() {
    const fragment = document.createDocumentFragment(); state.room.prizes.forEach(prize => { const chip = document.createElement("span"); chip.className = "prize-chip"; chip.append(document.createTextNode(`${prize.name} × ${prize.quantity}`)); const tag=document.createElement("i"); const rarity=prize.rarity||"standard"; tag.className=`rarity-tag rarity-${rarity}`; tag.textContent=rarityLabel(rarity); chip.append(tag); fragment.append(chip); }); elements.prizeLegend.replaceChildren(fragment);
  }

  function rarityLabel(rarity) { return rarity === "limited" ? "限定" : rarity === "rare" ? "稀有" : "常规"; }
  ...
  function animateCard(cardNumber, result) { return new Promise(resolve => { const card = elements.cardGrid.querySelector(`[data-card="${cardNumber}"]`); if (!card) return resolve(); const back = card.querySelector(".card-back"); const rarity=prizeRarity(result.prize_name); back.classList.toggle("winner", Boolean(result.prize_name)); if(result.prize_name)back.classList.add(`rarity-${rarity}`); back.querySelector(".result-icon").textContent = result.prize_name ? "✦" : "○"; back.querySelector("strong").textContent = state.name; back.querySelector("span:last-child").textContent = result.prize_name || "本次未抽中"; requestAnimationFrame(() => { card.classList.add("flipped", "just-flipped"); if (navigator.vibrate) navigator.vibrate(result.prize_name ? [35,35,70] : 25); }); setTimeout(resolve, 850); }); }
  ...
  function showReveal(result) { const won = Boolean(result.prize_name); const rarity=prizeRarity(result.prize_name); elements.revealKicker.textContent = won ? `${rarityLabel(rarity)}款式` : `第 ${result.card_number} 个盒子`; elements.revealTitle.textContent = won ? `抽到「${result.prize_name}」啦！` : "这个盒子没有礼物"; elements.revealText.textContent = won ? "稀有度只改变卡面颜色，不会影响实际抽取概率。" : "本次没有抽中，结果也会保留在开盒记录里。"; elements.revealIcon.textContent = won ? "✦" : "○"; const box=elements.revealModal.querySelector(".reveal-box"); box.classList.remove("rarity-standard","rarity-rare","rarity-limited"); box.classList.toggle("win",won); if(won)box.classList.add(`rarity-${rarity}`); elements.revealModal.classList.remove("hidden"); if (won) launchCelebration(); elements.closeRevealButton.focus(); }
